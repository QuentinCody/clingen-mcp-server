...
interface QueryEnv {
    CLINGEN_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv) {
    const handler = createQueryDataHandler("CLINGEN_DATA_DO", "clingen");

    server.registerTool(
        "clingen_query_data",
        {
            title: "Query Staged ClinGen Data",
            description:
                "Query staged ClinGen data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
